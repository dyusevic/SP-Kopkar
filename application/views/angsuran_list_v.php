		...
		<td>
			<table style="width:100%">
				...
				<tr>
					<td> Biaya Admin</td>
					<td> : </td>
					<td class="h_kanan"> <?php echo number_format(($row_pinjam->biaya_adm)); ?></td>
				</tr>
				<tr>
					<td> Jasa Pinjaman</td>
					<td> : </td>
...
<thead>
	<tr>
		...
		<th data-options="field:'bayar_jasa_pinjaman', width:'20', halign:'center', align:'right'">Jasa Pinjaman</th>
...
<div id="dialog-form" class="easyui-dialog" modal="true" show="blind" hide= "blind" resizable="false" style="width:400px; height:460px; padding: 20px 20px" closed="true" buttons="#dialog-buttons" style="display: none;">
			...
			<tr style="height:30px">
				...
				<tr style="height:30px">
					...
					<td> <div class="inputform"><?php echo number_format((nsi_round($row_pinjam->ags_per_bulan))); ?></div>
						<input type="hidden" id="pokok" name="pokok" value="<?php echo ($row_pinjam->ags_per_bulan); ?>"/>
						<input type="hidden" id="total_bayar_" name="total_bayar_" value="<?php echo ($total_bayar); ?>" />
					</td>
				</tr>
				<tr style="height:30px">
					<td> Jasa Pinjaman</td>
					...
					<td>
						<?php
						$jumlah_angs = (nsi_round(($row_pinjam->bunga * $row_pinjam->jumlah * 0.01)));
						?>
						<input type="text" id="bunga" name="bunga" value="<?php echo $jumlah_angs; ?>"/>
					</td>
				</tr>
				<tr style="height:30px">
					<td> Jumlah Angsuran</td>
					<td> : </td>
					<td> 
						<div class="inputform" style="display: none;"><span id="jml_angsuran"></span></div>
						<input type="text" id="angsuran" name="angsuran" value=""/>
					</td>
				</tr>
				<tr style="height:30px">
					<td> Sisa Tagihan</td>
					<td> :</td>
					<td> 
						<span id="sisa_tagihan" class="inputform"></span>
						<!-- <input type="text" id="sisa_tagihan_" name="sisa_tagihan_" value="" /> -->
					</td>
	...
	$(document).ready(function() {
		...
		var angsuran;
		var total_bayar_;
		bunga = $("#bunga").val().replace(',','');
		pokok = $("#pokok").val().replace(',','');
		angsuran = $('#angsuran').val().replace(',','');
		total_bayar_ = $('#total_bayar_').val();


		sisa = total_bayar_ - angsuran;
		$('#sisa_tagihan').text(sisa);

		hasil = parseInt(bunga) + parseInt(pokok);
		$("#jml_angsuran").html(hasil);
		// $("#angsuran").val(hasil);
		...
		// 		hasil = parseInt(bunga) + parseInt(pokok);
		// 	}else{
		// 		hasil = parseInt(pokok);
		// 	}
		// 	$("#jml_angsuran").html(hasil);
		// 	var angsuran = $("#angsuran").val(hasil);

		// 	var total_bayar_ 	= $('#total_bayar_').val();
		// 	var sisa 			= total_bayar_ - hasil;
		// 	$('#sisa_tagihan').text(sisa);
		// });

		$('#angsuran').keyup(function(){
			var angsuran = $('#angsuran').val().replace(',','');
			// var angsuran = $("#angsuran").val();
			var total_bayar_ 	= $('#total_bayar_').val();

			var sisa 			= total_bayar_ - angsuran;
			$('#sisa_tagihan').text(sisa);
		});
	...
	function create() {
		...
		var pokok = $("#pokok").val().replace(',','');
		var total_bayar_ = $('#total_bayar_').val();
		jQuery('#angsuran').val(pokok);
		...
		var sisa = total_bayar_ - angsuran;
		$('#sisa_tagihan').text(total_bayar_);
		$.ajax({
			type	: "POST",
			url		: "<?php echo site_url('angsuran/get_ags_ke') . '/'.$master_id.''; ?>",
			success	: function(result){
	...
	function save(){
		...
		if(kas_id == 0){
			...
			if(angsuran == 0 || angsuran == ''){
				$.messager.show({
					title:'<div><i class="fa fa-warning"></i> Peringatan ! </div>',
					msg: '<div class="text-red"><i class="fa fa-ban"></i> Maaf, Jumlah Angsuran tidak boleh kosong </div>',
					timeout:2000,
					showType:'slide'
				});
				$("#kas_id").focus();
