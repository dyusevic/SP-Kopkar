...
class Pinjaman_m extends CI_Model {
	...
	function get_pengajuan() {
		...
		$sql_tampil = "SELECT 
			...
			b.identitas AS identitas, b.nama AS nama
	...
	function get_jenis_pinjaman_pj() {
		$this->db->select('*');
		$this->db->from('tbl_barang');
		$query = $this->db->get();
		if($query->num_rows()>0){
	...
	function lap_data_pinjaman_by_id($jenis_id) {
		$kode_transaksi = isset($_REQUEST['kode_transaksi']) ? $_REQUEST['kode_transaksi'] : '';
		$cari_status = isset($_REQUEST['cari_status']) ? $_REQUEST['cari_status'] : '';
		$tgl_dari = isset($_REQUEST['tgl_dari']) ? $_REQUEST['tgl_dari'] : '';
		$tgl_sampai = isset($_REQUEST['tgl_sampai']) ? $_REQUEST['tgl_sampai'] : '';
		$sql = '';
		$sql = " SELECT * FROM v_hitung_pinjaman WHERE dk = 'K' AND barang_id = ".$jenis_id;
		$q = array('kode_transaksi' => $kode_transaksi, 
			'cari_status'	=> $cari_status,
			'tgl_dari' 		=> $tgl_dari, 
			'tgl_sampai' 	=> $tgl_sampai);
	...
	public function create() {
		...
		$data = array(			
			...
			'jns_pinjam'			=>	$this->input->post('jenis_pinj'),
			'no_pinjaman'			=>	$this->input->post('no_pinjam'),
	...
	public function get_anggota($postData){
		$this->db->where('id', $postData['anggota_id']);
	...
	public function get_kode_jenis_pinjaman($postData){
		$this->db->where('id', $postData['jenis_pinjaman']);
		return $this->db->get('jns_pinjam')->result_array();
	}

	public function get_lama_angsuran($postData){
		$masa_kerja = $postData['masa_kerja'];
		$query = $this->db->query("SELECT * FROM `jns_angsuran` WHERE ".$masa_kerja." >= masa_kerja AND ".$masa_kerja." <= masa_kerja_max");
		return $query->result_array();
		// $this->db->where("$postData['masa_kerja'] >= masa_kerja");
